...
function userInit() {
    ...
    if (localStorage.user) {
        ...
        user = new User(+localStorage.points, +localStorage.totalEarnedPoints, +localStorage.totalLostPoints, +localStorage.completedTasks, +localStorage.failedTasks, +localStorage.completedMissions, +localStorage.committedOffenses, questManager);
        ...
        console.log("created a new user and loaded into localstorage. welcome.");
    }
...
function addQuestsFromLocalStorage(questManager) {
    ...
    types.forEach((type) => {
        JSON.parse(localStorage.getItem(type)).forEach((quest) => {
            questManager.addQuest(type, quest.name, quest.desc, quest.reward, quest.penalty, false);
        });
    });
}
